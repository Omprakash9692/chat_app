  ...
  const handleConfirmAdd = async () => {
    ...
    if (result?.success) {
      if (result.isPending) {
        showToast(
          "Request Sent",
          `Join request sent for ${selectedMemberIds.length} member(s). Admin approval is required.`,
          "info",
        );
      } else {
        showToast(
          "Members Added",
          `Added ${selectedMemberIds.length} member(s) to the group.`,
          "success",
        );
      }
      ...
      showToast(
        "Error",
        result?.message || "Could not add members to group.",
        "danger",
      );
    }
  };
