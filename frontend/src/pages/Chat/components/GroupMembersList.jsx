                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      type="button"
                      onClick={async () => {
                        const success = await handleJoinRequest(
                          group.id,
                          reqUserRealId,
                          "approve",
                        );
                        if (success) {
                          showToast(
                            "Member Approved",
                            `${reqUser.name} has been added to the group.`,
                            "success",
                          );
                        }
                      }}
          ...
          return (
            ...
            >
              <div
                ...
                className={`flex items-center justify-between p-3 rounded-2xl bg-white border border-slate-200/80 shadow-2xs transition-all ${
                  !isMe
                    ? "cursor-pointer hover:bg-slate-50 hover:border-[#008069]/40"
                    : "cursor-default"
                  ...
                  {amIAdmin && (
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        const targetId = member._id?.toString() || member.id;
                        if (isTargetAdmin) {
                          const success = await dismissGroupAdmin(
                            group.id,
                            targetId,
                          );
                          if (success) {
                            ...
                            showToast(
                              "Action Failed",
                              "Could not revoke admin status.",
                              "danger",
                            );
                          }
                        } else {
                          const success = await makeGroupAdmin(
                            group.id,
                            targetId,
                          );
                          if (success) {
                            ...
                            showToast(
                              "Action Failed",
                              "Could not assign admin permissions.",
                              "danger",
                            );
                          }
                        }
                  ...
                  {amIAdmin && (
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        setActiveMemberMenuId(null);
                        const targetId = member._id?.toString() || member.id;
                        const success = await removeFromGroup(
                          group.id,
                          targetId,
                        );
                        if (success) {
                          showToast(
                            "Member Removed",
                            `${member.name} was removed from space.`,
                            "warning",
                          );
                        } else {
                          showToast(
