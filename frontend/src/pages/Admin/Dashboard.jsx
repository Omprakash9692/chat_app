...
export const Dashboard = () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.success && result.data?.groups)
          setGroupsList(result.data.groups);
      }
    } catch (err) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.success && result.data?.reports)
          setAdminReports(result.data.reports);
      }
    } catch (err) {
  ...
  const handleToggleBlockUserConfirmed = (
    userId,
    userName,
    isCurrentlyBlocked,
  ) => {
    setConfirmModal({
      ...
      message: isCurrentlyBlocked
        ...
        : `Are you sure you want to ban ${userName}? They will be logged out immediately.`,
      ...
      onConfirm: async () => {
        try {
          ...
          if (res.ok) {
            ...
            showToast(
              isCurrentlyBlocked ? "User Unbanned" : "User Banned",
      ...
      onConfirm: async () => {
        try {
          ...
          if (res.ok) {
            ...
            showToast(
              "User Deleted",
              `${userName} was deleted from database.`,
              "success",
            );
          } else {
  ...
  const handleToggleBlockGroupConfirmed = (
    groupId,
    groupName,
    isCurrentlyBlocked,
  ) => {
    setConfirmModal({
      ...
      message: isCurrentlyBlocked
        ? `Are you sure you want to lift suspension on "${groupName}"?`
        : `Are you sure you want to suspend "${groupName}"? Members won't be able to send messages.`,
        ...
        try {
          ...
          if (res.ok) {
            ...
            const updatedBlocked =
              result.data?.group?.isBlocked ?? !isCurrentlyBlocked;
            setGroupsList((prev) =>
              prev.map((g) =>
                g.id === groupId ? { ...g, isBlocked: updatedBlocked } : g,
              ),
            );
            showToast(
              updatedBlocked ? "Group Suspended" : "Group Unblocked",
              `"${groupName}" status updated.`,
              updatedBlocked ? "warning" : "success",
            );
            void fetchAdminStats();
          } else {
            ...
            showToast(
              "Action Failed",
              errJson.message || "Failed to update group status.",
              "error",
            );
          }
        } catch {
      ...
      onConfirm: async () => {
        try {
          ...
          if (res.ok) {
            ...
            showToast(
              "Action Failed",
              errJson.message || "Failed to delete group.",
              "error",
            );
          }
        } catch {
  ...
  const updateReportStatus = async (reportId, status) => {
    ...
    try {
      ...
      showToast(
        "Error",
        `Failed to ${status === "resolved" ? "resolve" : "dismiss"} report.`,
        "error",
      );
    } finally {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center bg-slate-50 p-6 text-center select-none">
        ...
        <p className="text-xs text-slate-500 max-w-sm mt-1 mb-6 font-medium leading-relaxed">
          You need administrator rights to view compliance analytics and manage
          platform accounts.
        </p>
      </div>
    );
  ...
  const realUsersList = (allUsers || []).filter(
    (u) => u.role !== "Admin" && u.role !== "admin",
  );
  const realBlockedUsersCount = realUsersList.filter(
    (u) => u.isBlocked || u.statusText === "Blocked",
  ).length;
  const realBlockedGroupsCount = (groupsList || []).filter(
    (g) => g.isBlocked,
  ).length;
  const realPendingReportsCount = (adminReports || []).filter(
    (r) => r.status === "pending",
  ).length;
  ...
  const cardStats = [
    {
      ...
      icon: Users,
      subtext: `${realBlockedUsersCount} Banned`,
    ...
    },
    {
      ...
      icon: MessageSquare,
      subtext: `${realBlockedGroupsCount} Suspended`,
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs select-none">
          <motion.div
            ...
            className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200/80 text-left space-y-4"
          >
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              <div
                className={`p-2.5 rounded-2xl ${confirmModal.variant === "danger" ? "bg-rose-50 text-rose-600" : "bg-amber-50 text-amber-600"}`}
              >
        ...
        <div className="text-left w-full lg:w-auto shrink-0 flex items-center justify-between">
          <div>
            ...
            <p className="text-[9px] text-indigo-600 font-bold uppercase tracking-[0.24em] mt-1">
              Admin Audit Panel — Realtime System Insights
            </p>
          </div>
        </div>
              ...
              {cardStats.map((stat, idx) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div
                      className={`h-9 w-9 rounded-xl ${stat.bg} ${stat.border} border flex items-center justify-center ${stat.color} shadow-xs`}
                    >
