          ...
          <table className="w-full min-w-170 border-collapse text-left">
            ...
            <tbody className="divide-y divide-slate-100 text-xs font-semibold">
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((u) => {
                  const isUserBlocked =
                    u.statusText === "Blocked" || u.isBlocked;
                  return (
                    <tr
                      key={u.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 select-all font-mono font-medium text-[11px] text-slate-700">
                        <span className="bg-slate-50 border border-slate-200/50 px-2 py-1 rounded-md">
                          {u.email}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 select-none">
                        <span
                          className={`inline-flex items-center gap-2 text-xs font-extrabold ${isUserBlocked ? "text-rose-500" : "text-emerald-500"}`}
                        >
                          <span
                            className={`h-2 w-2 rounded-full ${isUserBlocked ? "bg-rose-500" : "bg-emerald-500"} ${!isUserBlocked && "animate-pulse"}`}
                          />
                        ...
                        {u.id !== "user_me" && (
                          <>
                            <button
                              ...
                              title={
                                isUserBlocked ? "Unban Account" : "Ban Account"
                              }
                            >
                              {isUserBlocked ? (
                            ...
                            <button
                              onClick={() =>
                                handleDeleteUserConfirmed(u.id, u.name)
                              }
