...
export const MultiMetricBarChart = ({
  weekData,
  monthDataMap,
  availableMonths,
  yearData,
  ...
  React.useEffect(() => {
    if (availableMonths && availableMonths.length > 0) {
      const exists = availableMonths.some(
        (m) => m.index === selectedMonthIndex,
      );
      if (!exists) {
        setSelectedMonthIndex(
          availableMonths[availableMonths.length - 1].index,
        );
      }
    }
  }, [availableMonths]);
  ...
  if (timeframe === "week") {
    ...
    const rawMonthObj = monthDataMap
      ? monthDataMap[selectedMonthIndex] ||
        monthDataMap[String(selectedMonthIndex)]
      : null;
    ...
    if (selectedWeek === "all") {
      ...
      activeData =
        ...
        monthObj.daysByWeek[selectedWeek].length > 0
          ? monthObj.daysByWeek[selectedWeek]
          : getMonthWeeksFallback(selectedMonthIndex).daysByWeek[
              selectedWeek
            ] || [];
    }
  } else {
  ...
  const selectedMonthObj =
    activeMonthsList.find((m) => m.index === selectedMonthIndex) ||
    activeMonthsList[activeMonthsList.length - 1];
  ...
  const isTotalZero = data.every(
    (d) =>
      (d.users || 0) === 0 && (d.groups || 0) === 0 && (d.reports || 0) === 0,
  );
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 select-none">
        <div>
          ...
          <p className="text-[11px] text-slate-500 font-bold mt-0.5 flex items-center gap-1.5">
            ...
            {timeframe === "month" &&
              selectedWeek === "all" &&
              `Showing 4 Weeks of ${selectedMonthObj?.name || "Month"} 2026 (Click any week bar for daily breakdown)`}
            {timeframe === "month" &&
              selectedWeek !== "all" &&
              `Showing All Days of ${selectedWeek.toUpperCase()} in ${selectedMonthObj?.name || "Month"} 2026`}
            {timeframe === "year" &&
              `Showing Real Database Metrics for 2026 (Jan – ${latestMonthObj?.shortName || "Aug"})`}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1.5 bg-slate-900 text-white px-3 py-1.5 rounded-2xl shadow-xs">
                ...
                >
                  ...
                  <option value="week1" className="text-slate-900 font-medium">
                    Week 1 (Days 1–7)
                  </option>
                  <option value="week2" className="text-slate-900 font-medium">
                    Week 2 (Days 8–14)
                  </option>
                  <option value="week3" className="text-slate-900 font-medium">
                    Week 3 (Days 15–21)
                  </option>
                  <option value="week4" className="text-slate-900 font-medium">
                    Week 4 (Days 22–28)
                  </option>
                  <option value="week5" className="text-slate-900 font-medium">
                    Week 5 (Days 29–31/Rest)
                  </option>
                </select>
              </div>
        ...
        <div className="min-w-150 sm:min-w-full h-full">
          ...
          >
            ...
            {data.map((d, i) => {
              ...
              const barW = Math.max(
                4,
                (availableW - (activeSeriesCount - 1) * gap) /
                  activeSeriesCount,
              );
              const groupW =
                activeSeriesCount * barW + (activeSeriesCount - 1) * gap;
              ...
              return (
                <g
                  ...
                  className={
                    timeframe === "month" && selectedWeek === "all"
                      ? "cursor-pointer"
                      : ""
                  }
                >
                    ...
                    return (
                      <g key={s.key}>
                        <rect
                          ...
                          style={{
                            filter: isHovered
                              ? `drop-shadow(0 0 6px ${s.color}80)`
                              : "none",
                          }}
                        />
                      </g>
            ...
            {data.map((d, i) => (
              <span
                ...
                className={`flex-1 text-center truncate px-0.5 ${
                  hoverIndex === i ? "text-indigo-600 font-black scale-110" : ""
            ...
            <div>
              <span className="text-[10px] uppercase tracking-widest text-indigo-400 font-black">
                {timeframe === "week"
                  ? "Day View"
                  : timeframe === "month"
                    ? `${selectedMonthObj?.name || "Month"} (${selectedWeek === "all" ? "Overview" : selectedWeek.toUpperCase()})`
                    : `${displayYear} Year View`}
              </span>
              <h4 className="text-sm font-black text-white">
                {activePoint.date &&
                activePoint.date.startsWith(activePoint.name)
                  ? activePoint.date
